...
class FaceRecognitionTest extends TestCase
{
    ...
    public function testRecognizeFaceWithUrl()
    {
        $mockResponse = $this->responseFaceRecognitionWithUrl();
        $this->client->pushResponse($mockResponse);

        $imageUrl = 'http://my-domain.com/path/to/image.jpg';

        $this->api->detectFaces($imageUrl);
    ...
    public function testRecognizeFaceWithFile()
    {
        $mockResponse = $this->responseFaceRecognitionWithFile();
        $this->client->pushResponse($mockResponse);
